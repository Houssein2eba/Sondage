...
$user=$_SESSION["user"];
$sql="SELECT id_user from users where nom='$user'";
$res=mysqli_query($conn,$sql);
if(!$res){
 die("users error".mysqli_error($conn));
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    str int(1) not null,id_user int, FOREIGN KEY (id_user) REFERENCES users(id_user))";
    $r = mysqli_query($conn, $sql);
    if (!$r) {
        echo 'Error creating table: ' . mysqli_connect_error($conn);
    }
    $sql = "INSERT INTO $tab (str,id_user) VALUES (?,?)";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("ii", $str,$id);
        $stmt->execute();
        $stmt->close();
    } else {
...
</div><input type="text" class="tab" name="tab" value="mab">
     <h1>Mohamed Abdalahi </h1>
         <div class="bt" >
...
<button class="btn-danger btn"><a href="http://localhost/prep/users/logout.php" style="text-decoration: none;">logout</a></button>
</body>
</html>
