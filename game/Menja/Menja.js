...
const state = {
	game: {
		mode: GAME_MODE_RANKED, time: 0, score: 0, cubeCount: 0 },
	menus: {
		active: MENU_MAIN } };
...
const makeCooldown = (rechargeTime, units=1) => {
	...
	const updateTime = () => {
		...
		else {timeRemaining -= now-lastTime;
...
timeRemaining -= rechargeTime-options.rechargeTime;
		...
		reset() {timeRemaining = 0;
			lastTime = 0;this.mutate(initialOptions); } };
...
function makeCubeModel({ scale=1 }) {
	return {
		vertices: [
			{ x: -scale, y: -scale, z: scale }, { x:  scale, y: -scale, z: scale }, { x:  scale, y:  scale, z: scale }, { x: -scale, y:  scale, z: scale },
			{ x: -scale, y: -scale, z: -scale }, { x:  scale, y: -scale, z: -scale }, { x:  scale, y:  scale, z: -scale }, { x: -scale, y:  scale, z: -scale } ],
		polys: [
			{ vIndexes: [0, 1, 2, 3] }, { vIndexes: [7, 6, 5, 4] }, { vIndexes: [3, 2, 6, 7] }, { vIndexes: [4, 5, 1, 0] }, { vIndexes: [5, 6, 2, 1] },
			{ vIndexes: [0, 3, 7, 4] } ] }; }
...
function makeRecursiveCubeModel({ recursionLevel, splitFn, color, scale=1 }) {
	...
	cubeOrigins.forEach((origin, cubeIndex) => {
		const occlusion = Math.max(
			Math.abs(origin.x),
			Math.abs(origin.y),
			Math.abs(origin.z) ) / maxComponent;
...
function mengerSpongeSplit(o, s) {
	return [
		{ x: o.x + s, y: o.y - s, z: o.z + s }, { x: o.x + s, y: o.y - s, z: o.z + 0 }, { x: o.x + s, y: o.y - s, z: o.z - s }, { x: o.x + 0, y: o.y - s, z: o.z + s }, { x: o.x + 0, y: o.y - s, z: o.z - s }, { x: o.x - s, y: o.y - s, z: o.z + s }, { x: o.x - s, y: o.y - s, z: o.z + 0 }, { x: o.x - s, y: o.y - s, z: o.z - s },

		{ x: o.x + s, y: o.y + s, z: o.z + s }, { x: o.x + s, y: o.y + s, z: o.z + 0 }, { x: o.x + s, y: o.y + s, z: o.z - s }, { x: o.x + 0, y: o.y + s, z: o.z + s }, { x: o.x + 0, y: o.y + s, z: o.z - s }, { x: o.x - s, y: o.y + s, z: o.z + s }, { x: o.x - s, y: o.y + s, z: o.z + 0 }, { x: o.x - s, y: o.y + s, z: o.z - s },

		{ x: o.x + s, y: o.y + 0, z: o.z + s }, { x: o.x + s, y: o.y + 0, z: o.z - s }, { x: o.x - s, y: o.y + 0, z: o.z + s }, { x: o.x - s, y: o.y + 0, z: o.z - s } ]; }
...
class Entity {
	constructor({ model, color, wireframe=false }) {
		...
		const polys = model.polys.map(p => ({
			vertices: p.vIndexes.map(vIndex => vertices[vIndex]), color: color, wireframe: wireframe, strokeWidth: wireframe ? 2 : 0, strokeColor: colorHex, strokeColorDark: darkColorHex, depth: 0, middle: { x: 0, y: 0, z: 0 }, normalWorld: { x: 0, y: 0, z: 0 }, normalCamera: { x: 0, y: 0, z: 0 } }));
...
const getTarget = (() => {
	...
	function getTargetOfStyle(color, wireframe) {
		...
		if (!target) {target = new Entity({
				...
				color: color,
				wireframe: wireframe });
target.color = color;target.wireframe = wireframe;
target.hit = false;target.maxHealth = 0;target.health = 0; }
		return target; }
	...
	return function getTarget() {
		...
		if (spinner) {
			spinSpeeds[0] = -0.25;
			spinSpeeds[1] = 0;target.rotateZ = random(0, TAU); }
...
const createBurst = (() => {
	...
	return (target, force=1) => {
		transformVertices (
			basePositions, positions, target.x, target.y, target.z, target.rotateX, target.rotateY, target.rotateZ, 1, 1, 1 );
		transformVertices (
			basePositions, prevPositions, target.x - target.xD, target.y - target.yD, target.z - target.zD, target.rotateX - target.rotateXD, target.rotateY - target.rotateYD, target.rotateZ - target.rotateZD, 1, 1, 1);
		...
		transformVertices(
			...
			0, 0, 0,target.rotateX, target.rotateY, target.rotateZ, 1, 1, 1 );
...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		cubeCountNode.style.opacity = 1; } else {
		scoreNode.innerText = `النقاط : ${state.game.score}`;
		scoreNode.style.display = 'block';
		cubeCountNode.style.opacity = 0.65 ; }
	cubeCountNode.innerText = `المكعبات المحطمة : ${state.game.cubeCount}`; }
...
function renderMenus() {
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			if (isNewHighScore()) {
				highScoreLblNode.textContent = '!نقطة عالية جديدة'; }
			else {
				highScoreLblNode.textContent = `أعلى نقطة : ${formatNumber(getHighScore())}`; }
...
function tick(width, height, simTime, simSpeed, lag) {
	...
	for (let i = targets.length - 1; i >= 0; i--) {
		...
		if (target.z < backboardZ) {target.z = backboardZ;target.zD *= -boundDamping; }
		...
		if (target.y > centerY + targetHitRadius * 2) {targets.splice(i, 1);
			returnTarget(target);
			if (isInGame()) {
				if (isCasualGame()) {
					incrementScore(-25); } 
	...
	transformVertices(
		allShadowVertices, allShadowVertices, 0, 0, 0, TAU/8, 0, 0, 1, 1, 1 );
...
function handleCanvasPointerUp() {
	if (pointerIsDown) {
		pointerIsDown = false;
		touchPoints.push({touchBreak: true,
			life: touchPointLife });
...
if ('PointerEvent' in window) {
	...
	document.body.addEventListener('mouseleave', handleCanvasPointerUp); } else {
...
document.body.onmousedown = function(e) {
    var audio = document.getElementById("SSW");
    audio.play(); }